...
export const AdminDashboard: React.FC<Props> = ({ data, lang, onClear, onRefresh, isLoading, onLogout }) => {
  ...
  const exportToWord = (content: string, filename: string, titleSuffix: string = "") => {
    const formattedContent = content
      .replace(/^# (.*$)/gim, '<h1 style="color: #1e3a8a; text-align:center;">$1</h1>')
      .replace(/^## (.*$)/gim, '<h2 style="color: #1e3a8a; border-bottom: 2px solid #1e3a8a; margin-top:20px;">$1</h2>')
    ...
    const htmlContent = `
      ...
      <body style="font-family: 'Times New Roman', serif; line-height: 1.6;">
        <div style="text-align: center; border-bottom: 1px solid #000; padding-bottom: 10px; margin-bottom: 20px;">
          ...
          <p style="font-weight: bold;">RASMIY HISOBOT - ${titleSuffix}</p>
        </div>
        <div>${formattedContent}</div>
        <p style="margin-top: 40px; font-size: 10pt;">Yaratildi: ${new Date().toLocaleString()}</p>
      </body>
      </html>
    `;
  ...
  const handleAiAnalysis = async () => {
    ...
    } catch (e) { alert("AI tahlilda xato!"); } 
  ...
  const handleIndividualAi = async () => {
    ...
    } catch (e) { alert("Individual tahlilda xato!"); } 
    finally { setLoadingIndividualAi(false); }
  };

  const getRiskBadge = (score: number) => {
    if (score >= 2.5) return <span className="px-3 py-1 bg-red-100 text-red-700 rounded-full text-[10px] font-black uppercase">{t.highRisk}</span>;
    if (score >= 1.5) return <span className="px-3 py-1 bg-amber-100 text-amber-700 rounded-full text-[10px] font-black uppercase">{t.mediumRisk}</span>;
    return <span className="px-3 py-1 bg-emerald-100 text-emerald-700 rounded-full text-[10px] font-black uppercase">{t.lowRisk}</span>;
  };
    ...
    <div className="max-w-7xl mx-auto p-4 space-y-10 pb-32 fade-in">
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center glass-morphism p-10 rounded-[2.5rem] shadow-xl border border-white/50 gap-6">
        <div>
          ...
          <p className="text-blue-700 font-black text-sm uppercase tracking-widest mt-2">{t.totalSurveys}: {data.length}</p>
        </div>
        <div className="flex flex-wrap gap-4">
          <button onClick={() => exportToWord(`## Umumiy statistika\nJami so'rovlar: ${data.length}`, 'Statistika_Hisoboti', 'Monitoring Statistika')} className="px-6 py-4 bg-blue-600 text-white rounded-2xl font-bold hover:bg-blue-700 transition shadow-lg">📄 {t.exportData}</button>
      ...
      <div className="glass-morphism p-8 rounded-[2.5rem] shadow-lg border border-white/50">
        ...
        {/* Xatoni tuzatish: div balandligi va minWidth 0 */}
        <div className="h-[400px] w-full min-w-0" style={{ minHeight: '400px' }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={statsByClass} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
      ...
      {/* List / Group View Switch */}
      <div className="flex bg-gray-100 p-1 rounded-2xl border border-gray-200 shadow-inner w-fit mx-auto">
        <button onClick={() => setViewMode('list')} className={`px-10 py-3 rounded-xl font-bold text-xs transition-all ${viewMode === 'list' ? 'bg-white text-blue-600 shadow-md' : 'text-gray-500 hover:text-gray-700'}`}>{t.listView}</button>
        <button onClick={() => setViewMode('group')} className={`px-10 py-3 rounded-xl font-bold text-xs transition-all ${viewMode === 'group' ? 'bg-white text-blue-600 shadow-md' : 'text-gray-500 hover:text-gray-700'}`}>{t.groupView}</button>
      </div>
              ...
              <tbody className="divide-y divide-gray-200">
                {data.map((r) => (
                  <tr key={r.id} className="hover:bg-blue-50/80 transition-colors">
                    ...
                    <td className="px-10 py-6 text-black/60 text-xs font-bold">{formatDate(r.timestamp)}</td>
                    <td className="px-10 py-6 text-center">{getRiskBadge(getAverageScore(r.answers))}</td>
                    <td className="px-10 py-6 text-right">
                      <button onClick={() => setSelectedResponse(r)} className="px-6 py-2 bg-black text-white rounded-xl font-black text-xs hover:bg-gray-800 transition shadow-md">{t.viewDetails}</button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="grid grid-cols-1 gap-6 fade-in">
          {Object.entries(groupedData).map(([school, classes]) => (
            <div key={school} className="glass-morphism rounded-[2.5rem] overflow-hidden border border-white/50 shadow-lg">
              <button onClick={() => setExpandedSchool(expandedSchool === school ? null : school)} className="w-full p-8 flex justify-between items-center bg-white/40 hover:bg-white/60 transition">
                ...
                <div className="p-8 space-y-4 border-t border-white/40 bg-white/20">
                  {Object.entries(classes).map(([className, students]) => (
                    <div key={className} className="bg-white/60 rounded-3xl border border-white overflow-hidden shadow-sm">
                      <button onClick={() => setExpandedClass(expandedClass === `${school}-${className}` ? null : `${school}-${className}`)} className="w-full px-6 py-5 flex justify-between items-center hover:bg-white/80 transition">
                        <span className="text-sm font-black text-black">{className} sinf ({students.length} ta o'quvchi)</span>
                      ...
                      {expandedClass === `${school}-${className}` && (
                        <div className="px-6 pb-6 pt-2 divide-y divide-gray-100">
                          {students.map(student => (
                            <div key={student.id} className="py-4 flex justify-between items-center">
                              ...
                              <button onClick={() => setSelectedResponse(student)} className="px-4 py-2 bg-gray-100 hover:bg-black hover:text-white transition rounded-xl text-[10px] font-black uppercase">{t.viewDetails}</button>
                            </div>
                          ))}
                        </div>
                      )}
      ...
      <div className="glass-morphism p-10 rounded-[2.5rem] shadow-xl border border-white/50">
        <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-6">
          <h3 className="text-3xl font-black text-black flex items-center gap-4"><span className="p-3 bg-indigo-100 rounded-2xl text-2xl">🤖</span> {t.aiAnalysis}</h3>
          <div className="flex flex-wrap gap-3">
            <button onClick={handleAiAnalysis} disabled={loadingAi} className="px-10 py-4 bg-indigo-700 text-white rounded-2xl font-black hover:bg-indigo-800 transition flex items-center gap-3 shadow-lg disabled:bg-indigo-300">
            ...
            {aiAnalysis && (
              <div className="flex bg-white/50 border border-indigo-100 rounded-2xl p-1 gap-1">
                ...
                <button onClick={() => exportToWord(aiAnalysis, "AI_Hisobot", "Monitoring AI Xulosasi")} className="px-6 py-3 bg-blue-100 text-blue-800 rounded-xl font-bold border border-blue-200">📄 Word</button>
                <button onClick={() => shareContent(aiAnalysis, "Monitoring AI Xulosasi")} className="px-6 py-3 bg-emerald-100 text-emerald-800 rounded-xl font-bold border border-emerald-200">📤 Ulashish</button>
              </div>
            )}
          </div>
        </div>
        {aiAnalysis && <div className="prose max-w-none bg-white p-10 rounded-[2rem] border border-gray-200 whitespace-pre-wrap text-black font-bold fade-in shadow-inner overflow-y-auto max-h-[600px] custom-scrollbar leading-relaxed">{aiAnalysis}</div>}
      </div>
          ...
          <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden max-h-[95vh] flex flex-col border border-gray-100">
            ...
            <div className="p-10 overflow-y-auto space-y-8 custom-scrollbar">
              <button onClick={handleIndividualAi} disabled={loadingIndividualAi} className="w-full py-5 bg-indigo-700 text-white rounded-2xl font-black shadow-lg hover:bg-indigo-800 transition disabled:bg-indigo-300">
                {loadingIndividualAi ? "Psixologik tahlil tayyorlanmoqda..." : "✨ Individual AI Tahlil"}
              </button>
              {individualAiResult && (
                <div className="space-y-3">
                  <div className="p-8 bg-indigo-50/50 rounded-[2rem] text-sm whitespace-pre-wrap leading-relaxed shadow-inner border border-indigo-100 text-black font-bold">{individualAiResult}</div>
                  <div className="flex bg-white border border-indigo-100 rounded-2xl p-1 gap-1">
                    ...
                    <button onClick={() => exportToWord(individualAiResult, `${selectedResponse.user.firstName}_Xulosa`, "Individual Tahlil")} className="flex-1 py-3 bg-blue-50 text-blue-800 rounded-xl border border-blue-100">📄 Word</button>
              ...
              <div className="space-y-4">
                ...
                {SURVEY_QUESTIONS.map(q => (
                  <div key={q.id} className="p-4 bg-gray-50 rounded-xl border border-gray-100">
                    <p className="text-sm font-bold text-gray-800 mb-2">{q.text[lang]}</p>
